      ...
      const showFeedback = (isCorrect, correctAnswer) => {
        ...
        container.innerHTML = `
          <div style="text-align:center;padding:60px 20px;font-family:system-ui,-apple-system,sans-serif;">
            <div style="width:80px;height:80px;background:${bgColor};border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">${icon}</div>
        ...
        <div style="font-family:system-ui,-apple-system,sans-serif;padding:8px;">
          ...
          <div style="background:#F3F4F6;border-radius:8px;padding:12px;margin-bottom:16px;">
            <p style="font-size:13px;color:#6B7280;display:flex;align-items:center;gap:6px;">
              ${svgIcon('timer', { size: 14, color: '#6B7280' })}
              Durée estimée du quiz : <strong id="qte-estimate">${(currentData.timePerCard * cards.length / 60).toFixed(1)} min</strong>
            </p>
          </div>
